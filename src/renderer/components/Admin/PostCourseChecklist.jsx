...
const PostCourseChecklist = ({ register, user, onBackToList, openSignatureModal }) => {
    ...
    useEffect(() => {
        const fetchChecklistData = async () => {
            ...
            const courseDocIds = register.courseDocIds ? register.courseDocIds.split(',').map(Number) : [];
            const applicableDocIds = POST_COURSE_DOC_IDS.filter(id => courseDocIds.includes(id));
    ...
    const handleSubmitChecklist = async () => {
        ...
        try {
            await window.db.run(
                "UPDATE datapack SET status = 'complete' WHERE id = @param1",
                [register.id]
            );
            setAlertInfo({
                show: true,
                title: 'Success!',
                message: 'Post-course checklist submitted successfully! The event is now complete.'
            });
        } catch (error) {
    ...
    if (isLoading) {
        return <div className="p-6 text-center">Loading Post-Course Checklist...</div>;
    }
        ...
        <>
            ...
            <div className="flex flex-col h-full bg-gray-50">
                <div className="p-4 border-b bg-white">
                    ...
                    <h1 className="text-2xl font-bold text-gray-800">Post-Course Checklist for {register.courseName}</h1>
                </div>
                <div className="flex flex-grow overflow-hidden">
                    <div className="w-1/4 bg-gray-100 p-4 border-r overflow-y-auto flex flex-col justify-between">
                        ...
                        <div>
                            ...
                            >
                                {allDocumentsCompleted ? 'Submit Post-Course Checklist' : 'Checklist Incomplete'}
                            </button>
                        </div>
                    </div>
                    ...
                    <main className="w-3/4 p-6 overflow-y-auto">
                        {selectedDocument && eventDetails && user ? (
                            <QuestionnaireForm
                                key={`${selectedDocument.id}-${eventDetails.id}`}
                                user={user}
...
export default PostCourseChecklist;
