...
export const generatePdf = async (datapackId, documentName, TemplateComponent) => {
    if (!datapackId) {
    ...
    try {
        ...
        // 3. Prepare base props for the template
        const templateProps = {
        ...
        // 4. Fetch document-specific data and add to props
        if (documentName === 'TrainingCourseChecklist') {
            const document = (await window.db.query('SELECT * FROM documents WHERE name = ?', [documentName]))[0];
            ...
            templateProps.comments = responses.reduce((acc, res) => {
        ...
        // 5. Render the React component to an HTML string
        const htmlContent = ReactDOMServer.renderToStaticMarkup(
            <TemplateComponent {...templateProps} />
        );

        // 6. Send the HTML to the main process for PDF generation and opening
        await window.electron.generatePdfFromHtml(htmlContent, datapack.id);

    } catch (error) {
        console.error(`Failed to generate ${documentName} PDF:`, error);
